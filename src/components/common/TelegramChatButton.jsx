  ...
  return (
    <div className="fixed right-4 bottom-20 lg:bottom-6 z-40 flex flex-col items-end gap-2">
      ...
      <button
        ...
        className="relative w-14 h-14 flex items-center justify-center drop-shadow-lg"
        aria-label="Xabar yozing"
      >
        {open ? (
          <span className="w-14 h-14 bg-accent text-white rounded-full flex items-center justify-center hover:bg-red-700 transition-colors">
            <X className="w-6 h-6" />
          </span>
        ) : <img src="/telegram_logo.svg" alt="Telegram" className="w-14 h-14" />}
        {!open && unreadCount > 0 && (
